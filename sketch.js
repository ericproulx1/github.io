...
function draw() {
  ...
  if (safeMode === false) {
    ...
    if (keyIsDown(88) || keyIsDown(120)) { //x for brake
      me.deccelerate();
      image(brake, window.innerWidth/4, window.innerHeight/4, 20 , 20); //make brake smaller, appear depressed
    } else {
      image(brake, window.innerWidth/4, window.innerHeight/4, 20, 20);
    }
